...
if __name__ == '__main__':
    ...
    #df = pd.read_csv('AdviceForTeens_data.csv', encoding= 'ISO-8859-1', low_memory=False)
    df = pd.read_csv('careerguidance_data.csv', encoding= 'unicode_escape')
    ...
    for sentence in sentences:
        ...
        ratings.append(str(vs))
        comp.append(vs['compound'])

    data = pd.DataFrame(list(zip(posts, ratings, comp)), columns=['Post', 'Intent Analysis', 'Intent'])
    print(data)
    data.to_csv('demo_results.csv')
