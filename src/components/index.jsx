...
const Index = () => {
  ...
  useEffect(() => {
    const fetchHistoryData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/getHistory?username=${userName}`);
            ...
            <div className="grid">
                <div id="sec1" className="section left">
                    {loggedIn ? (
                        ...
                        <Login onSuccessfulLogin={handleSuccessfulLogin} onLoginFailure={handleLoginFailure} />
                    )}
                    {showAlert && (
